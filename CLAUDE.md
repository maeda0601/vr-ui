...
- **固定ジェスチャーは2方式**（`freeze_gesture`）: `approach`（既定）は下記の自動固定。`middle` は親指＋中指ピンチ（`_lock_on`）を固定とみなし `_arm_on` に流し込む。固定の開始は「中指距離 < 人差し指距離×0.8」のときだけ（親指を人差し指に付けると中指にも近づくため）、保持中は距離のヒステリシスのみ。固定中の左クリックは「人差し指距離 ≤ 中指距離×1.3」（3本指でつまんだ形）で判定。チョキ（SCROLL）は固定中・中指ピンチ中は除外。このとき左クリックは `_lock_on` 中のみ、`arm_cancel_px`/`arm_timeout_sec` の自動解除は行わない（`state.lock_tip` が非None で判別）。`right_click_finger` が `middle` のときは衝突するので `approach` にフォールバック。
